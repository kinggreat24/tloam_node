...
namespace tloam
{
  ...
  {
    static size_t file_index = file_index_;
  ...
  bool KittiReader::initScanFilenames()
  {
    ...
    for (const auto &path : listPath)
    {
      ...
      if (baseName == "dataset")
      {
        ...
        std::cout << "velodyne_path: " << velodyne_path << std::endl;
        ...
        std::cout << "image_0_path: " << image_0_path << std::endl;
        ...
        std::string calibPath = path + "/calib/" + seq_num_ + "/calib.txt";
        std::cout << "calibPath: " << calibPath << std::endl;
        ...
        std::string gnss_path = path + "/poses/" + seq_num_ + ".txt";
        std::cout << "gnss_path: " << gnss_path << std::endl;
        if (boost::filesystem::exists(gnss_path))
        {
          readPose(gnss_path, ground_truth_);
        }
        std::cout << "ground_truth_ size: " << ground_truth_.size() << std::endl;
        // std::cout<<"pose 10: "<<std::endl<<ground_truth_[10].matrix()<<std::endl;
      }
    }
  ...
  {
    // std::cout << "scan_filenames_: " << scan_filenames_[index] << std::endl;
